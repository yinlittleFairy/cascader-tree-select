...
class HTMLScriptElementUtility {
    ...
    static async loadExternalScript(element) {
        ...
        if (element.ownerDocument.defaultView.happyDOM.settings.disableJavaScriptFileLoading ||
            ...
            this.onError(element, new DOMException_1.default(`Failed to load external script "${src}". JavaScript file loading is disabled.`, DOMExceptionNameEnum_1.default.notSupportedError));
            return;
        }
        if (async) {
            ...
            let code = null;
            try {
                code = await ResourceFetch_1.default.fetch(element.ownerDocument, src);
            }
            catch (error) {
            ...
            let code = null;
            try {
                code = ResourceFetch_1.default.fetchSync(element.ownerDocument, src);
            }
            catch (error) {
                this.onError(element, error);
                return;
